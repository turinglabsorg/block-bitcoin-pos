...
const checkInternet = async () => {
  try {
    ...
    if (seedConfirmed.value) {
      clearInterval(checkInterval)
      userChoice.value = 'proceed'
    }
  } catch (e) {
    if (!seed.value) {
      generateMnemonic()
...
const checkInterval = setInterval(checkInternet, 1000)
...
const confirmSeed = async () => {
  if (seedConfirm.value === seed.value) {
    ...
    const root = bip32.fromSeed(recreatedSeed);
...
<template>
  <div class="init-container">
    ...
    <div v-if="userChoice === 'generator'">
      ...
      <div class="step-text" v-if="!theresInternet && seedConfirmed">
        <h2>Please put your internet connection on again to proceed.</h2>
      </div>
      ...
      <div v-if="showMnemonicConfirmation && !seedConfirmed">
        ...
        <button class="restart-button full" @click="showMnemonicConfirmation = false">Go back</button>
